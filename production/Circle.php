...
if (!empty($_POST)) {
    if (isset($_POST['token']) && $_POST['token'] === $_SESSION['token']) {
        $post=$db->prepare('INSERT INTO circles SET created_by=?, post=?, created=NOW()');
...
$posts=$db->query('SELECT m.name, p.* FROM members m  JOIN circles p ON m.id=p.created_by ORDER BY p.created DESC');
